...
`, attachment: fs.createReadStream(__dirname + "/cache/ullash.mp4")} ,threadID));
        ...
        try {
            ...
            const randomPath = readdirSync(join(__dirname, "cache", "ar3.png", "joinGif", "randomgif"));
 
            if (existsSync(pathGif)) formPush = { body: msg, attachment: createReadStream(pathvideo), mentions }
            else if (randomPath.length != 0) {
                const pathRandom = join(__dirname, "cache", "ar3.png", "joinGif", "randomgif", `${randomPath[Math.floor(Math.random() * randomPath.length)]}`);
